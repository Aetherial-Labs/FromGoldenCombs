...
namespace FromGoldenCombs.Blocks
{
        ...
        public void LoadTypes()
        {
            ...
            List<JsonItemStack> closedList = new();
            foreach (JsonItemStack stack in list)
            {
                if (stack.Attributes["type"].ToString() == "closed") closedList.Add(stack);
            }
            CreativeInventoryStacks = new CreativeTabAndStackList[1]
            {
                new()
                {
                    Stacks = closedList.ToArray(),
